...
- [ ] 仅在 CI 通过后创建 `v0.3.0` 标签。建立了可迁移的密钥管理流程时，建议
  对标签进行密码学签名，但发布不强制要求签名。
- [ ] 将 `v0.3.0` 发布为 GitHub Release，将公开路由和 HTTP 方法变更标记为
  破坏性变更；重点说明不可变系统角色、首位 `super_admin` 离线引导和
  ETag/`If-Match` 控制，并明确活跃 JTI 适配器仍在规划中、尚未实现。
