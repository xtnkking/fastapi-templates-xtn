...
role, authentication session, audit event, and business
...
- a UUID deterministically derived from an old integer, email, username, phone,
  business slug, or other guessable/private source.
...
named display field with an explicit uniqueness rule. It is never the
primary key, foreign key, JWT subject, or sole authorization lookup.

Human-readable role keys and permission keys such as `projects:read` are
...
as a security secret; use UUIDv4 for all user-controlled, session, audit,
and business rows.
...
it a UUIDv4 ID; do not switch to an integer. Name foreign keys and enforce the
unique relationship in PostgreSQL.

Create indexes for real access paths. Foreign keys used for joins need supporting
indexes, and chronological lists need deterministic ordering such as
`(created_at, id)`. Do not sort by UUID and pretend it is creation order. UUIDv4
index locality is the accepted security/default tradeoff; consider
...
`sub` is exactly `str(users.id)` and never a role-assignment ID, email, username, or
...
valid UUID exists before authentication, capability checks, and row-policy
evaluation.

UUIDs reduce trivial neighbor guessing, but they are not authorization. A leaked
UUID remains usable by an attacker unless every endpoint enforces capability and
object policy. Preserve generic `404` behavior for missing or deliberately
concealed IDs, rate-limit abuse, audit sensitive enumeration patterns, and test IDOR across
...
3. Backfill child UUID foreign keys by joining to the parent's old key and verify
   that the mapping is complete and unambiguous.
...
- Create rows through both SQLAlchemy and direct SQL where supported, and prove
  ...
  `uuid.UUID`.
- Assert every user, role, session, audit, and business model
  uses native UUID columns; association keys and foreign keys use matching types.
- Test malformed UUIDs, random unknown UUIDs, leaked UUIDs without permission,
  deliberately concealed objects, and every list/bulk/nested variant. Missing
  and concealed objects must have the same response when policy requires it.
