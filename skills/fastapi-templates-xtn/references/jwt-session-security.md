...
| `tid` | Canonical UUIDv4 tenant scope; required by this tenant-bound baseline and matched to both the route and active PostgreSQL membership |
...
protocol metadata; `tid` limits a stolen token to one tenant and is scope, not
profile data. A product that intentionally uses cross-tenant tokens may omit
`tid` only after replacing this binding with path-to-membership validation and
documenting the wider theft impact. Do not ask for that choice in the normal
tenant-RBAC case.

Never place username, email, phone, display name, role names, permission keys,
management tier, ownership, protection flags, account or membership status,
...
Use the stable user UUID as `sub`; never use an email, username, membership ID,
or auto-incremented integer. Only when choosing/changing the ID strategy or
...
- Before Redis or database access, verify signature, exact `iss` and `aud`, every
  ...
  UUIDv4 `sub`, `tid`, and `jti`. Use at most 30 seconds clock skew by default.
...
access token JTI. It binds `user_id`, `tenant_id`, status, issuance/expiry times,
...
`users.token_version`, and match row `user_id`/`tenant_id` to `sub`/`tid`. Querying
only by JTI is insufficient. Follow the concrete model in
...
- Namespace a key by environment and a SHA-256 digest of `issuer + NUL + jti`.
  Store no raw JWT. The value binds canonical `sub`, `tid`, literal access type,
  and `exp`.
...
-> verify signature, issuer, audience, type, times, sub, tid, and jti
-> require exact Redis active-JTI record
-> load matching active PostgreSQL session and active user
-> match route tenant to tid and load active membership
...
- A valid identity without a visible active tenant membership follows the RBAC
  `404` concealment rule. A visible identity lacking a permission receives `403`.
- A cryptographically valid, active token whose `tid` differs from the route
  tenant also returns that same generic `404`; do not classify the visible path
  tenant as a malformed credential.

Redis validation never replaces current PostgreSQL user, session, membership, or
RBAC checks. A valid JTI proves only that this credential remains registered.
...
1. Authenticate credentials and load the current active user and tenant
...
For an authorization control-plane or immediate-revocation business write, lock
and reload the user plus authentication session inside the authoritative
PostgreSQL transaction before tenant authorization locks and the final decision.
The entry Redis check is not that proof. Follow
...
- Accept a valid minimal token. Reject each missing required claim, prohibited
  ...
  non-canonical or non-v4 `sub`/`tid`/`jti`, invalid time type, overlong lifetime,
...
- A revoked or missing PostgreSQL session denies with a stale Redis key. Test one
  ...
  expiry, and membership suspension.
...
The current PostgreSQL asset requires `sub`, `tid`, `ver`, and `jti`, parses
`sub`/`tid` as generic UUIDs, and passes JTI through as a string. It therefore
accepts non-v4 or non-canonical subject/scope UUIDs and arbitrary JTI text. It has
no Redis dependency or `authentication_sessions` table, so it must not be
described as active-JTI revocation. Before production use, apply
[JWT session implementation](jwt-session-implementation.md): add both session
stores, remove `ver` from JWT in favor of server-side state, preserve `tid`
binding, and retain the transaction-local recheck for privileged writes.
